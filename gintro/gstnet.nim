...
proc newNetClientClock*(name: cstring = nil; remoteAddress: cstring;
    remotePort: int; baseTime: uint64): NetClientClock =
...
proc newNetClientClock*(tdesc: typedesc; name: cstring = nil; remoteAddress: cstring;
    remotePort: int; baseTime: uint64): tdesc =
...
proc initNetClientClock*[T](result: var T; name: cstring = nil; remoteAddress: cstring;
    remotePort: int; baseTime: uint64) {.deprecated.} =
  assert(result is NetClientClock)
  let gobj = gst_net_client_clock_new(name, remoteAddress, int32(remotePort), baseTime)
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
...
proc gst_net_time_packet_new(buffer: ptr array[16, uint8]): ptr NetTimePacket00 {.
    importc, libprag.}

proc newNetTimePacket*(buffer: ptr array[16, uint8]): NetTimePacket =
  fnew(result, gBoxedFreeGstNetTimePacket)
  result.impl = gst_net_time_packet_new(buffer)

proc newNetTimePacket*(tdesc: typedesc; buffer: ptr array[16, uint8]): tdesc =
  assert(result is NetTimePacket)
  fnew(result, gBoxedFreeGstNetTimePacket)
  result.impl = gst_net_time_packet_new(buffer)

proc initNetTimePacket*[T](result: var T; buffer: ptr array[16, uint8]) {.deprecated.} =
  assert(result is NetTimePacket)
  fnew(result, gBoxedFreeGstNetTimePacket)
  result.impl = gst_net_time_packet_new(buffer)
...
proc gst_net_time_packet_serialize(self: ptr NetTimePacket00): ptr array[16, uint8] {.
    importc, libprag.}

proc serialize*(self: NetTimePacket): ptr array[16, uint8] =
...
proc receive*(socket: gio.Socket; srcAddress: var gio.SocketAddress = cast[var gio.SocketAddress](nil)): NetTimePacket =
  var tmpoutgobjectarg: ptr gio.SocketAddress00
  var gerror: ptr glib.Error
  let impl0 = gst_net_time_packet_receive(cast[ptr gio.Socket00](socket.impl), cast[var ptr gio.SocketAddress00](if addr(srcAddress) == nil: nil else: addr tmpoutgobjectarg), addr gerror)
  if gerror != nil:
    let msg = $gerror.message
    g_error_free(gerror[])
    raise newException(GException, msg)
...
proc newNtpClock*(name: cstring = nil; remoteAddress: cstring; remotePort: int;
...
proc newNtpClock*(tdesc: typedesc; name: cstring = nil; remoteAddress: cstring; remotePort: int;
...
proc initNtpClock*[T](result: var T; name: cstring = nil; remoteAddress: cstring; remotePort: int;
...
proc bufferGetNetAddressMeta*(buffer: gst.Buffer): NetAddressMeta =
  let impl0 = gst_buffer_get_net_address_meta(cast[ptr gst.Buffer00](buffer.impl))
  if impl0.isNil:
    return nil
  new(result)
  result.impl = impl0
...
proc gst_ptp_init_full(config: ptr gst.Structure00): gboolean {.
    importc, libprag.}

proc ptpInitFull*(config: gst.Structure): bool =
  toBool(gst_ptp_init_full(cast[ptr gst.Structure00](config.impl)))
