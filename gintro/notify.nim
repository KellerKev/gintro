...
proc notify_notification_get_activation_token(self: ptr Notification00): cstring {.
    importc, libprag.}

proc getActivationToken*(self: Notification): string =
  let resul0 = notify_notification_get_activation_token(cast[ptr Notification00](self.impl))
  if resul0.isNil:
    return
  result = $resul0

proc activationToken*(self: Notification): string =
  let resul0 = notify_notification_get_activation_token(cast[ptr Notification00](self.impl))
  if resul0.isNil:
    return
  result = $resul0
...
proc setAppName*(self: Notification; appName: cstring = nil) =
  notify_notification_set_app_name(cast[ptr Notification00](self.impl), appName)

proc `appName=`*(self: Notification; appName: cstring = nil) =
  notify_notification_set_app_name(cast[ptr Notification00](self.impl), appName)
...
type
  ClosedReason* {.size: sizeof(cint), pure.} = enum
    unset = -1
    expired = 1
    dismissed = 2
    apiRequest = 3
...
const VERSION_MICRO* = 3'i32

const VERSION_MINOR* = 8'i32
...
proc init*(appName: cstring = nil): bool =
