...
// Original author: Stefan Salewski
// Revived and maintained by: Kevin Keller (KellerKev)
//Version 1.0.0

image:https://github.com/KellerKev/gintro/actions/workflows/ci.yml/badge.svg[CI,link=https://github.com/KellerKev/gintro/actions/workflows/ci.yml]

NOTE: *This is a revived fork of the original https://github.com/StefanSalewski/gintro[gintro] by Stefan Salewski,
which has been unmaintained since 2023.* This fork restores full compatibility with *Nim 2.x* (2.0+) on both
Linux and macOS, and is the recommended way to use GTK4/GTK3 bindings with modern Nim.

WARNING: *Nim 2.0 or later is required.* Support for Nim 1.x has been dropped as part of this revival.
If you need Nim 1.x support, use the original https://github.com/StefanSalewski/gintro[upstream fork].

*Install:*
[source,bash]
----
nimble install "https://github.com/KellerKev/gintro"
----

*What was fixed for Nim 2.x:*

* `tests/gen.nim` — tuple-unpacking from `seq[string]` (no longer valid in Nim 2.x)
* `tests/gen.nim` — generated `set[]` type aliases no longer carry `{.size: sizeof(cint).}` pragma
* `tests/maxby.nim` — `func` changed to `proc` for closure-parameter functions
* `gintro/gimplgobj.nim` — all tuple-unpacking from `split()` calls replaced with indexed access
* `gintro/gimplgobj.nim` — enum return types (e.g. `gst.FlowReturn`) handled correctly in signal callbacks
* `gintro/gimplglib.nim` — pure-enum set literal handling for `IOCondition` macros fixed for Nim 2.x
* `gintro.nimble` — post-generation patches for `gobject.nim`, `glib.nim`, `gst.nim`, `cairo.nim`
* `gobject.nim` — duplicate `IOCFlag`/`IOCondition` definitions guarded with `when not declared`

*WebKit bindings update:*

The GTK4 WebKit binding was previously `gintro/webkit25` (targeting the never-released WebKit2-5.0).
It has been updated to `gintro/webkit6` (WebKit-6.0, the GTK4 WebKit). If you were importing
`webkit25`, change your import to `webkit6`:

[source,nim]
----
# Old (broken):
import gintro/[gtk4, webkit25]
# New:
import gintro/[gtk4, webkit6]
----

*Known untested example — `examples/gtk3/fullgstcompiletest.nim`:*

This example imports every GStreamer binding module including `gstinsertbin`, `gstmpegts`,
`gstplayer`, and `gstwebrtc`. These modules require typelibs that are *not available* in the
standard `gir1.2-gst-plugins-bad-1.0` package on Ubuntu 22.04/24.04 and Debian-based distros.
The example is provided for reference only and is excluded from CI. If you need those modules,
you will need to build GStreamer bad plugins from source and ensure the corresponding `.typelib`
files are present in your `GI_TYPELIB_PATH`.
...
https://github.com/KellerKev/gintro/tree/master/examples . The github readme.adoc does not
...
* https://github.com/KellerKev/gintro
...
examples from https://github.com/KellerKev/gintro/tree/master/examples to your local computer, maybe to /tmp/gintro/examples directory.
